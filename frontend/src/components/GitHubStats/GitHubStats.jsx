...
function GHCard({ header, children, className = "" }) {
  ...
  return (
    ...
    >
      <div
        className="px-4 py-2.5 font-label-bold text-sm uppercase flex items-center justify-between border-b-4 border-(--color-outline) bg-surface-variant text-(--color-on-surface)"
      >
        {header}
      </div>
      {children}
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <GHCard
          header={<><span>Stats Overview</span><span className="opacity-60 text-xs">@{GITHUB_USERNAME}</span></>}
        >
          <div className="p-4 flex justify-center items-center min-h-50">
            <img src={statsUrl} alt="GitHub Stats" className="w-full h-auto" loading="lazy" />
          </div>
        </GHCard>

        <GHCard
          header={<><span>Top Languages</span><span className="text-xs opacity-60">By Repo</span></>}
        >
          <div className="p-4 flex justify-center items-center min-h-50">
            <img src={langsUrl} alt="Top Languages" className="w-full h-auto" loading="lazy" />
          </div>
        </GHCard>
      </div>
      ...
      <GHCard
        header={<><span>Contributions Calendar</span><span className="text-xs opacity-60">Commit History</span></>}

      >
        <div className="p-6 flex justify-center overflow-x-auto">
          <img src={contribUrl} alt="Contributions Calendar" className="min-w-162.5 max-w-full h-auto" loading="lazy" />
        </div>
      </GHCard>

      {/* Activity Graph */}
      <GHCard
        header={<><span>Contribution Trend</span><span className="opacity-60 text-xs">Activity Pulse</span></>}
      >
        <div className="p-4 flex justify-center" style={{ background: "#0d1117" }}>
          <img src={activityUrl} alt="Contribution Trend Graph" className="w-full max-w-3xl h-auto" loading="lazy" />
        </div>
      </GHCard>

      {/* Streak */}
      <GHCard
        header={<><span>Contribution Streak</span><span className="text-xs opacity-60">Consistency is key</span></>}

      >
        <div className="p-4 flex justify-center">
          <img src={streakUrl} alt="GitHub Streak" className="w-full max-w-2xl h-auto" loading="lazy" />
        </div>
      </GHCard>

      {/* Trophies */}
      <GHCard
        header={<><span>Trophies</span><span className="opacity-60 text-xs">Achievements Unlocked</span></>}
      >
        <div className="p-4 flex justify-center">
          <img src={trophiesUrl} alt="GitHub Trophies" className="w-full max-w-3xl h-auto" loading="lazy" />
        </div>
      </GHCard>
      ...
      <motion.a
        ...
        style={{ background: "var(--color-on-primary-container)", color: "#ffffff" }}
        variants={itemVariants}
